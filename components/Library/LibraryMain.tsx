...
const LibraryMain: React.FC<LibraryMainProps> = ({ items, isLoading, onRefresh, globalSearch }) => {
  ...
  // 1. Kolom header TYPE dihilangkan, digantikan SUB TOPIC
  ...
  return (
    <div className="flex flex-col h-full space-y-4 animate-in fade-in duration-500 overflow-visible relative">
        ...
        <div className="relative lg:hidden shrink-0">
          ...
          {showSortMenu && (
            <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-2xl border border-gray-100 z-[60] p-2 animate-in fade-in zoom-in-95">
              ...
              {/* 7. Mobile sort dropdown order update */}
              {[
                {k: 'title', l: 'Title'}, 
                {k: 'author', l: 'Author'}, 
                ...
                <button key={item.k} onClick={() => { handleSort(item.k as keyof LibraryItem); setShowSortMenu(false); }} className={`w-full text-left px-3 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-between ${sortConfig.key === item.k ? 'bg-[#FED400]/10 text-[#004A74]' : 'text-gray-500 hover:bg-gray-50'}`}>
                  <span>{item.l}</span>
              ...
              {paginatedItems.length === 0 ? (
                ...
                paginatedItems.map((item) => (
                  <StandardTr key={item.id} className="cursor-pointer" onClick={() => setSelectedItem(item)}>
                    <td className="px-6 py-4 sticky left-0 z-20 border-r border-gray-100/50 bg-white group-even:bg-[#fbfbfc] group-hover:bg-[#f0f7fa] shadow-sm" onClick={(e) => e.stopPropagation()}>
                      <StandardCheckbox checked={selectedIds.includes(item.id)} onChange={() => toggleSelectItem(item.id)} />
                    </td>
                    {/* 3 & 5. TITLE with icon and 2-line wrap */}
                    <StandardTd isActiveSort={sortConfig.key === 'title'} className="sticky left-12 z-20 border-r border-gray-100/50 bg-white group-even:bg-[#fbfbfc] group-hover:bg-[#f0f7fa] shadow-sm">
                      <div className="flex items-start gap-2 group/title">
                        <div className="shrink-0 mt-0.5">
                          {item.addMethod === 'FILE' ? (
                            <DocumentIcon className="w-4 h-4 text-[#FED400]" />
                          ) : (
                            <LinkIcon className="w-4 h-4 text-[#FED400]" />
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="text-sm font-bold text-[#004A74] line-clamp-2 group-hover/title:underline leading-snug">
                              {item.title}
                            </span>
      ...
      {/* 6. Mobile Card Structure Update */}
      <div className="flex lg:hidden flex-col flex-1 min-h-0 space-y-3 overflow-y-auto custom-scrollbar pb-10">
        {paginatedItems.map((item) => (
          <div key={item.id} className={`relative flex flex-col p-5 bg-white border rounded-3xl transition-all ${selectedIds.includes(item.id) ? 'border-[#004A74] shadow-md bg-[#004A74]/5' : 'border-gray-100 shadow-sm'}`} onClick={() => setSelectedItem(item)}>
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-[#FED400]">
            ...
            {/* Row 3: TITLE with icon */}
            <div className="flex items-start gap-2 mb-3">
              <div className="shrink-0 mt-1">
                {item.addMethod === 'FILE' ? (
                  <DocumentIcon className="w-3.5 h-3.5 text-[#FED400]" />
                ) : (
                  <LinkIcon className="w-3.5 h-3.5 text-[#FED400]" />
                )}
              </div>
              <div className="flex-1 min-w-0">
            ...
            {/* Selection handle for mobile */}
            <div className="absolute top-4 right-4" onClick={(e) => e.stopPropagation()}>
              <div className="opacity-0 w-0 h-0 overflow-hidden"><StandardCheckbox checked={selectedIds.includes(item.id)} onChange={() => toggleSelectItem(item.id)} /></div>
              <button 
